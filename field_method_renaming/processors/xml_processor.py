...
class XMLPatterns:
    ...
    def _build_field_patterns(self) -> List[Tuple[str, str]]:
        ...
        return [
            ...
            (r"t-if='([\s\S]*?){old_name}([\s\S]*?)'", r"t-if='\g<1>{new_name}\g<2>'"),
            (r't-elif="([\s\S]*?){old_name}([\s\S]*?)"', r't-elif="\g<1>{new_name}\g<2>"'),
            (r"t-elif='([\s\S]*?){old_name}([\s\S]*?)'", r"t-elif='\g<1>{new_name}\g<2>'"),
            (r't-unless="([\s\S]*?){old_name}([\s\S]*?)"', r't-unless="\g<1>{new_name}\g<2>"'),
            (r"t-unless='([\s\S]*?){old_name}([\s\S]*?)'", r"t-unless='\g<1>{new_name}\g<2>'"),
            
            # Attribute conditions - using [\s\S] to match across lines
            (r'invisible="([\s\S]*?){old_name}([\s\S]*?)"', r'invisible="\g<1>{new_name}\g<2>"'),
            (r"invisible='([\s\S]*?){old_name}([\s\S]*?)'", r"invisible='\g<1>{new_name}\g<2>'"),
            (r'readonly="([\s\S]*?){old_name}([\s\S]*?)"', r'readonly="\g<1>{new_name}\g<2>"'),
            (r"readonly='([\s\S]*?){old_name}([\s\S]*?)'", r"readonly='\g<1>{new_name}\g<2>'"),
            (r'required="([\s\S]*?){old_name}([\s\S]*?)"', r'required="\g<1>{new_name}\g<2>"'),
            (r"required='([\s\S]*?){old_name}([\s\S]*?)'", r"required='\g<1>{new_name}\g<2>'"),
            
            # Domain expressions - using [\s\S] to match across lines (like filter_domain)
            (r'domain="([\s\S]*?){old_name}([\s\S]*?)"', r'domain="\g<1>{new_name}\g<2>"'),
            (r"domain='([\s\S]*?){old_name}([\s\S]*?)'", r"domain='\g<1>{new_name}\g<2>'"),
            (r'filter_domain="([\s\S]*?){old_name}([\s\S]*?)"', r'filter_domain="\g<1>{new_name}\g<2>"'),
            (r"filter_domain='([\s\S]*?){old_name}([\s\S]*?)'", r"filter_domain='\g<1>{new_name}\g<2>'"),
            (r'context="([\s\S]*?){old_name}([\s\S]*?)"', r'context="\g<1>{new_name}\g<2>"'),
            (r"context='([\s\S]*?){old_name}([\s\S]*?)'", r"context='\g<1>{new_name}\g<2>'"),
            ...
            # Security references
            (r'field_id.*field_([^_]*)_{old_name}', r'field_id.*field_\g<1>_{new_name}'),
        ]
    ...
    def _build_method_patterns(self) -> List[Tuple[str, str]]:
        ...
        return [
            ...
            (r'onclick="([^"]*){old_name}(\([^"]*)"', r'onclick="\g<1>{new_name}\g<2>"'),
            (r"onclick='([^']*){old_name}(\([^']*)'", r"onclick='\g<1>{new_name}\g<2>'"),
            (r'onchange="([^"]*){old_name}(\([^"]*)"', r'onchange="\g<1>{new_name}\g<2>"'),
            (r"onchange='([^']*){old_name}(\([^']*)'", r"onchange='\g<1>{new_name}\g<2>'"),
...
class XMLProcessor(BaseProcessor):
    ...
    def _apply_field_changes(self, content: str, field_changes: Dict[str, str]) -> Tuple[str, List[str]]:
        """
        ...
        for old_name, new_name in field_changes.items():
            for pattern_template, replacement_template in self.patterns.field_patterns:
                ...
                new_content, count = re.subn(pattern, replacement, modified_content, flags=re.IGNORECASE | re.DOTALL)
                
                if count > 0:
                    modified_content = new_content
                    applied_changes.append(f"Field reference: {old_name} → {new_name} ({count} occurrences)")
                    logger.debug(f"Applied field pattern {pattern}: {count} replacements")
        
        return modified_content, applied_changes
    
    def _apply_method_changes(self, content: str, method_changes: Dict[str, str]) -> Tuple[str, List[str]]:
        """
        ...
        for old_name, new_name in method_changes.items():
            for pattern_template, replacement_template in self.patterns.method_patterns:
                ...
                if count > 0:
                    modified_content = new_content
                    applied_changes.append(f"Method reference: {old_name} → {new_name} ({count} occurrences)")
                    logger.debug(f"Applied method pattern {pattern}: {count} replacements")
        
        return modified_content, applied_changes
    
    def _apply_elementtree_changes(self, content: str, field_changes: Dict[str, str], 
        ...
        try:
            ...
            for field_elem in root.findall('.//field'):
                ...
                if name_attr and name_attr in field_changes:
                    ...
                    applied_changes.append(f"ElementTree field: {name_attr} → {field_changes[name_attr]}")
                    changes_made = True
            ...
            for button_elem in root.findall('.//button'):
                ...
                if name_attr and name_attr in method_changes:
                    ...
                    applied_changes.append(f"ElementTree button: {name_attr} → {method_changes[name_attr]}")
                    changes_made = True
                ...
                if action_attr and action_attr in method_changes:
                    ...
                    applied_changes.append(f"ElementTree action: {action_attr} → {method_changes[action_attr]}")
                    changes_made = True
                        ...
                        if ref_attr:
                            ...
                            for old_name, new_name in field_changes.items():
                                if ref_attr.endswith(f'_{old_name}'):
                                    new_ref = ref_attr.replace(f'_{old_name}', f'_{new_name}')
                                    field_elem.set('ref', new_ref)
                                    applied_changes.append(f"ElementTree ref: {ref_attr} → {new_ref}")
                                    changes_made = True
                        ...
                        if field_elem.text:
                            for old_name, new_name in field_changes.items():
                                if old_name in field_elem.text:
                                    field_elem.text = field_elem.text.replace(old_name, new_name)
                                    applied_changes.append(f"ElementTree field content: {old_name} → {new_name}")
                                    changes_made = True
    ...
    def _format_xml_output(self, xml_content: str) -> str:
        ...
        major_elements = ['record', 'data', 'odoo', 'template', 'xpath', 'form', 'tree', 'kanban', 'graph', 'pivot']
    ...
    def _filter_relevant_changes(self, file_path: Path, changes: List[FieldChange]) -> List[FieldChange]:
        ...
        try:
            ...
            for change in changes:
                ...
                if (model_key in models_in_file or 
                    change.model in models_in_file or
                    self._content_contains_reference(content, change.old_name)):
    ...
    def _extract_models_from_xml_file(self, content: str) -> Set[str]:
        ...
        try:
            ...
            try:
                ...
                logger.debug("Could not parse XML with ElementTree for model extraction")
    ...
    def validate_xml_changes(self, original_content: str, modified_content: str) -> bool:
        """
