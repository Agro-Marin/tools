...
class ProductTemplate(models.Model):
    ...
    product_properties = fields.Properties(
        'Properties', definition='categ_id.product_properties_definition', copy=True
    )
    ...
    is_dynamically_created = fields.Boolean(
        'Is Dynamically Created', compute='_compute_is_dynamically_created'
    )
                ...
                for combination in tmpl_id._filter_combinations_impossible_by_config(
                    ...
                    if combination in existing_variants:
                        ...
                        if len(current_variants_to_create) > int(variant_limit):
                            raise UserError(
                                _(
                                    'The number of variants to generate is above allowed limit. You should either not generate variants for each combination or generate them on demand from the sales order. To do so, open the form view of attributes and change the mode of *Create Variants*.'
                                )
                            )
    ...
    def _base_domain_item_ids(self):
        return ['|', ('pricelist_id', '=', False), ('pricelist_id.active', '=', True)]
    ...
    def default_get(self, fields):
        ...
        if (
            'uom_id' in fields
            and (not res.get('uom_id'))
            or self.env.context.get('default_uom_id') is False
        ):
    ...
    def _get_product_document_domain(self):
        self.ensure_one()
        return Domain('res_model', '=', 'product.template') & Domain(
            'res_id', 'in', self.ids
        ) | Domain('res_model', '=', 'product.product') & Domain(
    ...
    def _onchange_uom_id(self):
        ...
        return {'warning': {'title': _('What to expect ?'), 'message': message}}
