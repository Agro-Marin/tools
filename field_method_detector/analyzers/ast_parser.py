...
class OdooASTVisitor(ast.NodeVisitor):
    ...
    def _extract_model_name(self, node: ast.ClassDef) -> str | None:
    ...
    def _is_fields_call(self, node) -> bool:
        ...
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Attribute):
            if isinstance(node.func.value, ast.Name) and node.func.value.id == "fields":
                return True
        return False

    def _extract_field_info(self, node: ast.Assign) -> dict | None:
    ...
    def _extract_method_info(self, node: ast.FunctionDef) -> dict | None:
    ...
    def _generate_field_signature(
        self, field_type: str, args: list, kwargs: dict
    ) -> str:
    ...
    def _generate_method_signature(
        self, name: str, args: list[str], decorators: list[str]
    ) -> str:
