...
class TestCaseExtractor:
    ...
    def extract_case_from_commits(
        ...
        """
        logger.info(
            f"Extrayendo caso desde {commit_before[:8]} a {commit_after[:8]}..."
        )
        ...
        try:
            ...
            expected_output = self._run_detector_on_commits(
                commit_before, commit_after, getattr(self, "json_file", None)
            )
    ...
    def _run_detector_on_commits(
        self, commit_before: str, commit_after: str, json_file: Optional[str] = None
    ) -> List[Dict[str, Any]]:
    ...
    def _create_case_structure(
        ...
        return {
            ...
            "metadata": {
                ...
                "source_commits": {"before": commit_before, "after": commit_after},
    ...
    def _save_yaml_case(self, case_data: Dict[str, Any], output_file: str) -> bool:
        ...
        try:
            ...
            with open(output_path, "w", encoding="utf-8") as f:
                yaml.dump(
                    case_data, f, default_flow_style=False, allow_unicode=True, indent=2
                )
            ...
            for item in case["expected_output"]:
                ...
                for field in required_item_fields:
                    if field not in item:
                        logger.error(
                            f"Campo requerido faltante en expected_output: {field}"
                        )
                        return False
    ...
    def extract_critical_cases_from_csv(
        self, csv_file: str, output_dir: str, max_cases: int = 5
    ) -> List[str]:
        """
        ...
        try:
            with open(csv_file, "r", encoding="utf-8") as f:
                ...
                for row in reader:
                    ...
                    elif old_name.startswith("_compute") and new_name.startswith(
                        "_compute"
                    ):
...
Ejemplos de uso:
  ...
  python utils/test_case_extractor.py validate \\
    ...
    extract_parser = subparsers.add_parser(
        "extract-commits", help="Extraer caso desde commits Git"
    )
    extract_parser.add_argument(
        "--before", required=True, help="Commit antes de los cambios"
    )
    extract_parser.add_argument(
        "--after", required=True, help="Commit después de los cambios"
    )
    extract_parser.add_argument(
        "--name", required=True, help="Nombre descriptivo del caso"
    )
    extract_parser.add_argument(
        "--output", required=True, help="Archivo YAML de salida"
    )
    ...
    extract_parser.add_argument(
        "--main-script", default="main.py", help="Script principal del detector"
    )
    extract_parser.add_argument(
        "--json-file",
        help="Path al archivo JSON de módulos (requerido para detect_field_method_changes.py)",
    )

    # Comando: extract-from-csv
    csv_parser = subparsers.add_parser(
        "extract-from-csv", help="Extraer casos críticos desde CSV"
    )
    ...
    csv_parser.add_argument(
        "--max-cases", type=int, default=5, help="Máximo número de casos"
    )

    # Comando: validate
    validate_parser = subparsers.add_parser(
        "validate", help="Validar archivo YAML de caso"
    )
