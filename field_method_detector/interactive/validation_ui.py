...
class InteractiveValidator:
    ...
    def __init__(self, confidence_threshold: float = 0.75, auto_approve_threshold: float = 0.90):
        """
    ...
    def validate_candidates(self, candidates: List[RenameCandidate]) -> Tuple[List[RenameCandidate], Dict]:
        """
        ...
        auto_approved, needs_review, auto_rejected = self._classify_candidates(candidates)
        ...
        validation_summary = self._generate_validation_summary(
            auto_approved, manually_approved if needs_review else [],
            auto_rejected, len(approved_candidates)
        )
        
        return approved_candidates, validation_summary
    
    def _classify_candidates(self, candidates: List[RenameCandidate]) -> Tuple[List, List, List]:
    ...
    def _show_initial_summary(self, auto_approved: List, needs_review: List, auto_rejected: List):
...
""")
    ...
    def _show_auto_approved(self, auto_approved: List[RenameCandidate]):
        ...
        for candidate in auto_approved:
            print(f"  {candidate.old_name} → {candidate.new_name} ({candidate.module}.{candidate.model}) [{candidate.confidence:.1%}]")
    
    def _interactive_review(self, needs_review: List[RenameCandidate]) -> List[RenameCandidate]:
    ...
    def _prompt_user_validation(self, candidate: RenameCandidate, current: int, total: int) -> str:
        """
        ...
        confidence_bar = self._create_confidence_bar(candidate.confidence, self.auto_approve_threshold)
        ...
        print(f"📝 Cambio: {self.colors['error']}{candidate.old_name}{self.colors['reset']} → {self.colors['pass']}{candidate.new_name}{self.colors['reset']}")
        ...
        while True:
            ...
            if response in ['y', 'yes', 'sí', 'si']:
                print(f"✅ {self.colors['pass']}INCLUIDO{self.colors['reset']}: {candidate.old_name} → {candidate.new_name}")
                self.user_decisions.append({'candidate': candidate, 'decision': 'approved'})
                return 'approve'
            elif response in ['n', 'no', ''] or response == 'N':
                print(f"❌ {self.colors['error']}OMITIDO{self.colors['reset']}: {candidate.old_name} → {candidate.new_name}")
                self.user_decisions.append({'candidate': candidate, 'decision': 'rejected'})
                return 'reject'
    ...
    def _create_confidence_bar(self, confidence: float, threshold: float, bar_length: int = 30) -> str:
    ...
    def _show_detailed_analysis(self, candidate: RenameCandidate):
        ...
        print(f"   • Signature match: {'✅ Exacto' if candidate.signature_match else '❌ Diferente'}")
        ...
        print(f"   • Diferencia: {candidate.confidence - self.auto_approve_threshold:+.3f}")
...
{'-'*50}
    ...
    def _show_auto_rejected(self, auto_rejected: List[RenameCandidate]):
        ...
        for reason, candidates in rejection_groups.items():
            ...
            for candidate in candidates[:3]:  # Show first 3
                print(f"    • {candidate.old_name} → {candidate.new_name} [{candidate.confidence:.1%}]")
            if len(candidates) > 3:
                print(f"    ... y {len(candidates) - 3} más")
    
    def _generate_validation_summary(self, auto_approved: List, manually_approved: List,
                                   auto_rejected: List, total_approved: int) -> Dict:
        """Generate validation session summary"""
        return {
            'total_detected': len(auto_approved) + len(manually_approved) + len(auto_rejected),
    ...
    def show_final_summary(self, approved_candidates: List[RenameCandidate], 
                          validation_summary: Dict, output_file: str):
