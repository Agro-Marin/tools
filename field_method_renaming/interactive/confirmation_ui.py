...
class ConfirmationUI:
    ...
    def confirm_file_changes(self, file_path: Path, changes: List[FieldChange], 
                           preview_content: Optional[Tuple[str, str]] = None) -> bool:
        """
    ...
    def confirm_batch_changes(self, file_changes: Dict[Path, List[FieldChange]]) -> Dict[Path, bool]:
        """
    ...
    def display_dry_run_results(self, file_changes: Dict[Path, List[FieldChange]]):
        ...
        for module, stats in by_module.items():
            print(f"  {module}: {len(stats['files'])} files, {stats['changes']} changes")
    ...
    def _display_changes(self, changes: List[FieldChange]):
        ...
        if field_changes:
            ...
            for change in field_changes:
                print(f"    {change.old_name} → {change.new_name} ({change.module}.{change.model})")
    ...
    def _get_user_choice(self, prompt: str, options: List[str], 
