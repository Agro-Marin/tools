...
class BaseProcessor(ABC):
    ...
    def process_file(self, file_path: Path, changes: List[FieldChange]) -> ProcessResult:
        ...
        try:
            ...
            modified_content, applied_changes = self._apply_changes(file_path, original_content, relevant_changes)
            ...
            if self.validate_syntax and not self._validate_modified_syntax(file_path, modified_content):
            ...
            logger.info(f"Successfully processed {file_path}: {len(applied_changes)} changes applied")
    ...
    def get_processing_stats(self, results: List[ProcessResult]) -> Dict[str, any]:
        ...
        no_changes = len([r for r in results if r.status == ProcessingStatus.NO_CHANGES])
