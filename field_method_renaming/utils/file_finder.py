...
class FileSet:
    ...
    def all_files(self) -> List[Path]:
        ...
        return (self.python_files + self.view_files + self.data_files + 
                self.demo_files + self.template_files + self.report_files + 
                self.security_files)
    
    @property
    def xml_files(self) -> List[Path]:
        """Get all XML files"""
        return (self.view_files + self.data_files + self.demo_files + 
                self.template_files + self.report_files + self.security_files)
...
class FileFinder:
    ...
    def find_files_for_model(self, module: str, model: str) -> FileSet:
        ...
        if file_set.is_empty():
            logger.info(f"No files found with OCA conventions for {model}, trying recursive search...")
    ...
    def _search_oca_conventions(self, module_path: Path, patterns: Dict[str, List[str]]) -> FileSet:
        """
    ...
    def _pattern_matches_type(self, pattern: str, file_type: str) -> bool:
        ...
        """
        if file_type == 'views':
            return '_views.xml' in pattern or (not any(suffix in pattern for suffix in ['_data.xml', '_demo.xml', '_templates.xml', '_reports.xml', '_security.xml']))
        elif file_type == 'data':
    ...
    def _model_to_class_names(self, model: str) -> List[str]:
        ...
        camel_underscore = ''.join(word.capitalize() for word in underscore_name.split('_'))
    ...
    def _file_contains_model_reference(self, file_path: Path, search_terms: List[str]) -> bool:
        """
    ...
    def validate_module_exists(self, module: str) -> bool:
        ...
        return (module_path.exists() and 
                ((module_path / '__manifest__.py').exists() or 
                 (module_path / '__openerp__.py').exists()))