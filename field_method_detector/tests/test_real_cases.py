...
class TestRealCases:
    ...
    def __init__(
        self, repo_path: str = None, commit_before: str = None, commit_after: str = None
    ):
        ...
        if not self.detector_script.exists():
            raise FileNotFoundError(
                f"Script detector no encontrado: {self.detector_script}"
            )
    ...
    def _run_case(self, case_file: str):
        ...
        try:
            ...
            self._assert_detections_match(
                result_csv, case["expected_output"], case_file
            )
    ...
    def _validate_case(self, case: Dict[str, Any], case_file: str):
        ...
        if not (
            self.override_repo_path
            and self.override_commit_before
            and self.override_commit_after
        ):
            ...
            if not source_commits:
                raise ValueError(
                    f"Caso {case_file} falta información de commits en metadata.source_commits"
                )
    ...
    def _run_detector_on_case(self, case: Dict[str, Any]) -> str:
        ...
        if self.override_commit_before and self.override_commit_after:
            ...
            logger.info(
                f"🔧 Usando commits de parámetros: {commit_before[:8]}..{commit_after[:8]}"
            )
        else:
            ...
            logger.info(
                f"📋 Usando commits del YAML: {commit_before[:8]}..{commit_after[:8]}"
            )
        ...
        cmd = [
            ...
            str(self.json_file),
            "--repo-path",
            repo_path,  # Usar repo_path calculado
            "--commit-from",
            commit_before,
            "--commit-to",
            commit_after,
    ...
    def _assert_detections_match(
        self, result_csv: str, expected_output: List[Dict[str, Any]], case_file: str
    ):
        """
        ...
        for expected_item in expected_output:
            matching_detection = self._find_matching_detection(
                detections, expected_item
            )
    ...
    def _find_matching_detection(
        self, detections: List[Dict[str, str]], expected_item: Dict[str, Any]
    ) -> Dict[str, str] | None:
        """
        ...
        for detection in detections:
            if (
                detection.get("old_name") == expected_item["old_name"]
                and detection.get("new_name") == expected_item["new_name"]
                and detection.get("item_type") == expected_item["item_type"]
            ):
                return detection
...
Ejemplos de uso:
  ...
  python tests/test_real_cases.py \\
    ...
    parser.add_argument(
        "--repo", "--repo-path", help="Path al repositorio Git a analizar"
    )
    parser.add_argument(
        "--commit-before", "--before", help="Commit inicial (antes de los cambios)"
    )
    parser.add_argument(
        "--commit-after", "--after", help="Commit final (después de los cambios)"
    )
    parser.add_argument(
        "--case", help="Ejecutar solo un caso específico (ej: case_odoo_migration.yml)"
    )
    ...
    if args.commit_before or args.commit_after or args.repo:
        if not (args.commit_before and args.commit_after and args.repo):
            print(
                "❌ Error: Si usas --commit-before, --commit-after o --repo, debes especificar los tres"
            )
    ...
    try:
        ...
        if args.case:
            ...
            test_method = getattr(
                runner, f"test_{case_name.replace('case_', '')}", None
            )
