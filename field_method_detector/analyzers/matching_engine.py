...
class RenameCandidate:
    ...
    api_changes: Optional[Dict] = None
    file_path: str = ""
...
class MatchingEngine:
    ...
    def find_renames_in_inventories(self, before_inventory: Dict, after_inventory: Dict, 
                                   module_name: str, file_path: str = "") -> List[RenameCandidate]:
        """
        ...
        logger.debug(f"Fields processed: {len(before_inventory.get('fields', []))} → {len(after_inventory.get('fields', []))}")
        logger.debug(f"Methods processed: {len(before_inventory.get('methods', []))} → {len(after_inventory.get('methods', []))}")
        logger.debug(f"Total candidates found: {len(candidates)}")
        
        if candidates:
            for candidate in candidates:
                logger.debug(f"  Candidate: {candidate.old_name} → {candidate.new_name}")
    ...
    def _find_field_renames(self, fields_before: List[Dict], fields_after: List[Dict],
                           module_name: str, file_path: str) -> List[RenameCandidate]:
        """Find renamed fields using signature matching and naming rules"""
        candidates = []
        
        logger.debug(f"\n  === FIELD RENAME ANALYSIS ===")
        logger.debug(f"  Fields before: {[(f['name'], f.get('signature', 'no-sig')) for f in fields_before]}")
        ...
        for field_before in fields_before:
            ...
            if len(renamed_matches) == 1:
                ...
                logger.debug(f"Multiple signature matches for field {field_before['name']}: "
                ...
                if best_match:
                    ...
                    if validation['confidence'] >= 0.40:  # Lower threshold for disambiguated
                        candidate = RenameCandidate(
                            old_name=field_before['name'],
                ...
                fuzzy_matches = self._find_fuzzy_field_matches(field_before, fields_after)
                if fuzzy_matches:
                    logger.debug(f"Fuzzy matches for field {field_before['name']}: "
                               f"{[f['name'] for f in fuzzy_matches]}")
                    
                    # Try to find the best fuzzy match using naming rules
                    best_match = self._disambiguate_matches(field_before, fuzzy_matches, 'field')
                    if best_match:
                        validation = self._validate_field_rename(field_before, best_match)
                        if validation['confidence'] >= 0.60:  # Higher threshold for fuzzy matches
                            candidate = RenameCandidate(
                                old_name=field_before['name'],
                                new_name=best_match['name'],
                                item_type='field',
    ...
    def _find_method_renames(self, methods_before: List[Dict], methods_after: List[Dict],
                            module_name: str, file_path: str) -> List[RenameCandidate]:
        ...
        for method_before in methods_before:
            ...
            if len(renamed_matches) == 1:
                ...
                best_match = self._disambiguate_matches(method_before, renamed_matches, 'method')
                if best_match:
    ...
    def _find_signature_matches(self, target_item: Dict, candidate_items: List[Dict]) -> List[Dict]:
        """Find items with matching signatures"""
        matches = []
        target_signature = target_item.get('signature', '')
        
        logger.debug(f"    Looking for signature matches for '{target_item['name']}' (sig: {target_signature[:50]}...)")
        ...
        for candidate in candidate_items:
            ...
            if candidate_signature == target_signature:
                ...
                logger.debug(f"      ❌ No match: {candidate['name']} (sig: {candidate_signature[:50]}...)")
        
        logger.debug(f"    Total signature matches: {len(matches)}")
        return matches
    
    def _find_fuzzy_field_matches(self, target_field: Dict, candidate_fields: List[Dict]) -> List[Dict]:
        ...
        logger.debug(f"    Looking for fuzzy matches for '{target_name}' (type: {target_field_type})")
        ...
        for candidate in remaining_candidates:
            ...
            if self._are_field_types_compatible(target_field_type, candidate_field_type):
                ...
                if confidence > 0.30:  # Basic threshold for fuzzy matching
                    matches.append(candidate)
                    logger.debug(f"      ✅ Fuzzy match: {candidate_name} (type: {candidate_field_type}, confidence: {confidence:.3f})")
                else:
                    logger.debug(f"      ❌ Low confidence: {candidate_name} (type: {candidate_field_type}, confidence: {confidence:.3f})")
            else:
                logger.debug(f"      ❌ Incompatible type: {candidate_name} ({candidate_field_type} vs {target_field_type})")
    ...
    def _disambiguate_matches(self, target_item: Dict, matches: List[Dict], item_type: str) -> Optional[Dict]:
    ...
    def _validate_field_rename(self, field_before: Dict, field_after: Dict) -> Dict:
        ...
        if 'confidence' not in validation or validation['confidence'] is None:
            validation['confidence'] = self.calculate_comprehensive_confidence(validation)
        ...
        if field_before_type and field_after_type:
            if field_before_type == field_after_type:
                ...
                validation.setdefault('scoring_breakdown', {})['exact_type_match'] = 0.15
    ...
    def group_similar_renames(self, candidates: List[RenameCandidate]) -> Dict[str, List[RenameCandidate]]:
    ...
    def filter_high_confidence_renames(self, candidates: List[RenameCandidate], 
                                     threshold: float = 0.90) -> Tuple[List[RenameCandidate], List[RenameCandidate]]:
    ...
    def generate_confidence_summary(self, candidates: List[RenameCandidate]) -> Dict[str, Any]:
