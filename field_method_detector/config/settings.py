...
class Config:
    ...
    def __init__(self):
        self.confidence_threshold = float(os.getenv('CONFIDENCE_THRESHOLD', DEFAULT_CONFIDENCE_THRESHOLD))
        self.auto_approve_threshold = float(os.getenv('AUTO_APPROVE_THRESHOLD', AUTO_APPROVE_THRESHOLD))
        self.minimum_review_threshold = float(os.getenv('MINIMUM_REVIEW_THRESHOLD', MINIMUM_REVIEW_THRESHOLD))
        ...
        self.show_progress = os.getenv('SHOW_PROGRESS', str(SHOW_PROGRESS_BAR)).lower() == 'true'
    ...
    def validate(self):
        ...
        if self.auto_approve_threshold <= self.confidence_threshold:
            raise ValueError("Auto-approve threshold must be higher than confidence threshold")
