...
FIELD_NAMING_RULES = [
    # Counter fields: *_count → count_*
    {
        ...
        'examples': ['picking_ids_count → count_picking_ids', 'invoice_lines_count → count_invoice_lines'],
    ...
    # Order line fields: *order_line* → *line_ids*
    {
        ...
        'examples': ['order_line → line_ids', 'purchase_order_line → purchase_line_ids'],
        'weight': 0.9
    },
    ...
    # Tax inclusive suffixes: field_name → field_name_taxinc
    {
        ...
        'examples': ['amount_to_invoice → amount_to_invoice_taxinc', 'amount_invoiced → amount_invoiced_taxinc'],
...
METHOD_NAMING_RULES = [
    ...
    # Generic action methods: method_name → action_method_name (if not already prefixed and not covered by other patterns)
    {
        ...
        'examples': ['view_something → action_view_something', 'open_wizard → action_open_wizard'],
        'weight': 0.6
    },
    ...
    # Helper methods: get_*/prepare_*/find_* → _get_*/_prepare_*/_find_*
    {
        ...
        'examples': ['get_total → _get_total', 'prepare_values → _prepare_values', 'find_record → _find_record'],
        'weight': 0.6
    }
]
...
API_STYLE_PATTERNS = [
    # Old API → New API field types
    {
        ...
        'examples': ['fields.char() → fields.Char()', 'fields.many2one() → fields.Many2one()']
    }
]
...
class NamingRuleEngine:
    ...
    def apply_field_rule(self, old_name: str, field_type: Optional[str] = None) -> List[Dict]:
    ...
    def apply_method_rule(self, old_name: str, decorators: Optional[List[str]] = None) -> List[Dict]:
    ...
    def validate_field_conventions(self, field_name: str, field_type: str) -> List[Dict]:
    ...
    def detect_api_style_change(self, old_definition: str, new_definition: str) -> Optional[Dict]:
    ...
    def check_contextual_similarity(self, old_name: str, new_name: str) -> Optional[Dict]:
        """Check for contextual similarity patterns"""
        for pattern in self.contextual_patterns:
            if re.match(pattern['old_pattern'], old_name) and re.match(pattern['new_pattern'], new_name):
    ...
    def validate_rename(self, old_name: str, new_name: str, item_type: str, 
                       field_type: Optional[str] = None, 
                       decorators: Optional[List[str]] = None,
                       old_definition: str = "", 
                       new_definition: str = "") -> Dict:
        ...
        if item_type == 'field':
            ...
            for match in rule_matches:
                if match['predicted_name'] == new_name:
                    ...
                    validation_result['scoring_breakdown']['naming_rule'] = match['confidence']
                    break
            ...
            if field_type:
                ...
                convention_score = sum(v.get('bonus', v.get('penalty', 0)) for v in conventions)
                validation_result['scoring_breakdown']['convention_compliance'] = max(0, convention_score)
        ...
        if old_definition and new_definition:
            ...
            if api_change:
                ...
                validation_result['scoring_breakdown']['api_consistency'] = api_change.get('bonus', 0)
        ...
        if not validation_result['follows_rule']:
            ...
            if contextual:
                ...
                validation_result['scoring_breakdown']['contextual_match'] = contextual['confidence']
        ...
        validation_result['confidence'] = validation_result['confidence_score']  # Alias for consistency
