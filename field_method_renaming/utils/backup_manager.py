...
class BackupManager:
    ...
    def start_backup_session(self, session_name: Optional[str] = None) -> Path:
        """
    ...
    def create_backup(self, file_path: Path, content: Optional[str] = None) -> Path:
        """
    ...
    def create_batch_backup(self, file_paths: List[Path]) -> Dict[Path, Path]:
        ...
        logger.info(f"Batch backup completed: {len(backup_map)} successful, {len(failed_backups)} failed")
    ...
    def restore_session(self, session_dir: Path) -> Dict[str, bool]:
        ...
        try:
            ...
            for backup_dict in manifest.get('backups', []):
                ...
                success = self.restore_backup(backup_info.backup_path, backup_info.original_path)
                restoration_results[str(backup_info.original_path)] = success
            
            logger.info(f"Session restoration completed: {len(restoration_results)} files processed")
            ...
            if session_dir.is_dir():
                ...
                if manifest_path.exists():
                    try:
                        ...
                        sessions.append({
                            'session_name': manifest.get('session_name', session_dir.name),
                            'session_dir': session_dir,
                            'created_at': manifest.get('created_at'),
                            'backup_count': len(manifest.get('backups', [])),
                            'total_size': sum(b['file_size'] for b in manifest.get('backups', []))
                        })
                    except Exception as e:
                        logger.warning(f"Could not read manifest for session {session_dir}: {e}")
                else:
    ...
    def get_backup_statistics(self) -> Dict:
        ...
        try:
            disk_usage = sum(f.stat().st_size for f in self.backup_base_dir.rglob('*') if f.is_file())
        except Exception:
            disk_usage = 0
