...
class RenamingConfig:
    ...
    def __init__(self):
        # Core settings
        self.create_backups = self._get_bool_env(
            "CREATE_BACKUPS", DEFAULT_CREATE_BACKUPS
        )
        self.validate_syntax = self._get_bool_env(
            "VALIDATE_SYNTAX", DEFAULT_VALIDATE_SYNTAX
        )
        self.interactive_mode = self._get_bool_env(
        ...
        self.backup_retention_days = int(
            os.getenv("BACKUP_RETENTION_DAYS", DEFAULT_BACKUP_RETENTION_DAYS)
        )
        self.compress_backups = self._get_bool_env(
            "COMPRESS_BACKUPS", DEFAULT_COMPRESS_BACKUPS
        )

        # Processing settings
        self.file_types = self._get_list_env(
            "FILE_TYPES", list(FILE_TYPE_CATEGORIES.keys())
        )
    ...
    def should_process_file_type(self, file_type: str) -> bool:
        ...
        if "xml" in self.file_types and file_type in [
            "views",
            "data",
            "demo",
            "templates",
