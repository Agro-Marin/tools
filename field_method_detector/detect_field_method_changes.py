...
def analyze_module_files(
    ...
    for file_path in relevant_files:
        ...
        try:
            ...
            logger.debug(
                f"  Fields ({len(inventory_before.get('fields', []))}): {[f['name'] for f in inventory_before.get('fields', [])]}"
            )
            logger.debug(
                f"  Methods ({len(inventory_before.get('methods', []))}): {[m['name'] for m in inventory_before.get('methods', [])]}"
            )

            # Log inventory after
            logger.debug(f"COMMIT AFTER ({commit_to[:8]}):")
            logger.debug(
                f"  Fields ({len(inventory_after.get('fields', []))}): {[f['name'] for f in inventory_after.get('fields', [])]}"
            )
            logger.debug(
                f"  Methods ({len(inventory_after.get('methods', []))}): {[m['name'] for m in inventory_after.get('methods', [])]}"
            )
            ...
            if file_candidates:
                ...
                for candidate in file_candidates:
                    logger.debug(
                        f"   {candidate.old_name} → {candidate.new_name} (confidence: {candidate.confidence:.2f})"
                    )
            else:
